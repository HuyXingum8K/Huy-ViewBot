...
def title_loop():
    ...
    if os.name == "nt":
        while True:
            os.system(f'title TikTok Viewbot by @nxt_dev ^| success: {success} fails: {fails} reqs: {reqs} rps: {rps} rpm: {rpm}')
            time.sleep(0.1)

if __name__ == "__main__":
    os.system("cls" if os.name == "nt" else "clear"); os.system("title TikTok Viewbot by @nxt_dev" if os.name == "nt" else "")
    txt = """\n\n╦  ╦╦╔═╗╦ ╦╔╗ ╔═╗╔╦╗\n╚╗╔╝║║╣ ║║║╠╩╗║ ║ ║ \n ╚╝ ╩╚═╝╚╩╝╚═╝╚═╝ ╩ \n"""
    print(
        Colorate.Vertical(
